...
from torch_geometric.nn import GCNConv, GATConv, GATv2Conv, global_mean_pool, global_max_pool, SAGEConv, GINConv, GlobalAttention, GraphNorm
...
class SimpleGAT(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels=1, heads=4, num_layers=4, dropout=0.1):
        """
        Enhanced GAT model for fair comparison:
         - Multiple GATConv layers (default 4, same as SimpleGraphSAGE)
        ...
        for i in range(num_layers):
            if i == num_layers - 1:  # Last layer: single head, no concat
        ...
        # Global attention pooling (same as GraphSAGE)
        ...
        # Prediction head (same structure as GraphSAGE)
    ...
    def _init_weights(self):
        ...
        # Initialize final layer to predict reasonable speedup range
        with torch.no_grad():
            nn.init.normal_(self.head[-1].weight, mean=0.0, std=0.1)
            self.head[-1].bias.fill_(0.5)  # Start predicting around 1.6x speedup
...
class SimpleGCN(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels=1, num_layers=4, dropout=0.1):
        """
        Enhanced GCN model for fair comparison:
         - Multiple GCNConv layers (default 4, same as SimpleGraphSAGE)
         - Layer normalization for stability
         - Global attention pooling
         - Better initialization
        """
...
import torch.nn.functional as F
from torch_geometric.nn import GINConv, GraphNorm, GlobalAttention, SAGEConv
...
class PearlGATv2(nn.Module):
    ...
    def forward(self, data):
        if not PearlGATv2._printed_once:
            print("\n" + "="*50)
            print("--- DIAGNOSTIC: INSIDE PearlGATv2.forward() ---")
            print(f"--- Received Batch Object of Type: {type(data)} ---")
            print("\n--- Inspecting Feature Tensor (data.x) ---")
            print("Shape:", data.x.shape)
            print("First 5 rows:\n", data.x[:5])
            
            zero_percentage = ((data.x == 0).sum() / data.x.numel()) * 100
            print(f"\nPercentage of zeros: {zero_percentage:.2f}%")
            
            print("\n--- Inspecting Label Tensor (data.y) ---")
...
class SimpleGraphSAGE(nn.Module):
    """
    GraphSAGE implementation optimized for cost/speedup prediction.
    - Multiple SAGEConv layers with residual connections
    - Layer normalization for stability
    - Global attention pooling
    - Better initialization for regression tasks
    """
    def __init__(
        self,
        in_channels: int,
        ...
        # All layers use hidden_channels since we project input first
        for _ in range(num_layers):
        ...
        # Prediction head with better initialization
        self.head = nn.Sequential(
            nn.Linear(hidden_channels, hidden_channels // 2),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout),
    ...
    def forward(self, data):
        ...
        # DEBUG: Print data batch info (uncomment to debug)
...
class DeepGraphSAGE(nn.Module):
    """
    Stabilized Deep GraphSAGE with gradient clipping and simpler pooling.
    - Skip connections every 2 layers
    - Consistent mean aggregation for stability
    - Layer normalization instead of batch norm
    - Simplified global attention pooling
    """
    def __init__(
        self,
        in_channels: int,
        ...
        # GraphSAGE layers with consistent mean aggregation for stability
        self.convs = nn.ModuleList()
        self.norms = nn.ModuleList()
        
        for i in range(num_layers):
            # Use mean aggregation consistently for stability
            self.convs.append(SAGEConv(hidden_channels, hidden_channels, aggr="mean"))
            self.norms.append(nn.LayerNorm(hidden_channels))  # LayerNorm instead of BatchNorm
        
        self.dropout = nn.Dropout(dropout)
        
        # Simplified global attention pooling (single head for stability)
        self.gate_nn = nn.Sequential(
            nn.Linear(hidden_channels, hidden_channels // 2),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_channels // 2, 1),
        ...
        # Prediction head with gradient clipping friendly design
        self.head = nn.Sequential(
            nn.Linear(hidden_channels, hidden_channels // 2),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout),
    ...
    def forward(self, data):
        ...
        # GraphSAGE layers with skip connections every 2 layers
        skip_connections = []
        for i, (conv, norm) in enumerate(zip(self.convs, self.norms)):
            residual = x if i > 0 else None
            ...
            # Skip connection every 2 layers (more conservative)
            if i % 2 == 1 and residual is not None:
                x = x + residual
