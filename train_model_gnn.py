...
from utils_gnn.train_utils import train_gnn_model, mape_criterion, mse_criterion, log_mse_loss
...
def main(conf):
    ...
    gnn_dataset_train = GNNDatasetParallel(
        ...
        pkl_output_folder=f"{ROOT_DIR}/gnn_pickles/train",
        nb_processes=nb_processes,
        device="cpu",  # Use CPU for data preprocessing to avoid CUDA conflicts
    ...
    gnn_dataset_val = GNNDatasetParallel(
        ...
        pkl_output_folder=f"{ROOT_DIR}/gnn_pickles/val",
        nb_processes=nb_processes,
        device="cpu",  # Use CPU for data preprocessing to avoid CUDA conflicts
        just_load_pickled=just_load_pickled
    )
    ...
    best_loss, best_model_state = train_gnn_model(
        config=conf,
        model=model,
        criterion=mape_criterion,  # Optimize MAPE directly per request
        optimizer=optimizer,
