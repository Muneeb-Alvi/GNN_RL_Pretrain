...
def build_gnn_data_for_schedule(
    ...
    for loop_name in program_annot["iterators"]:
        ...
        # Derive basic loop stats from the tree structure
        tree = schedule_json.get("tree_structure", {})
        roots = tree.get("roots", []) if isinstance(tree, dict) else []
        
        # Compute depth and child count for this loop
        max_depth = 1
        depth_val = 0
        num_children = 0
        
        def walk(node, d):
            nonlocal depth_val, num_children, max_depth
            if node.get("loop_name") == loop_name:
                depth_val = d
                num_children = len(node.get("child_list", []))
            max_depth = max(max_depth, d)
            for ch in node.get("child_list", []):
                walk(ch, d + 1)
        
        for r in roots:
            walk(r, 0)
        
        # Parallelization flag: if any computation marks this loop as parallelized dim
        is_parallel = 0.0
        has_unroll = 0.0
        was_tiled = 0.0
        try:
            # schedule_json has per-computation schedule blocks
            for comp_name, sched in schedule_json.items():
                if not isinstance(sched, dict):
                    continue
                if sched.get("parallelized_dim") == loop_name:
                    is_parallel = 1.0
                if sched.get("unrolling_factor"):
                    has_unroll = 1.0
                if sched.get("tiling"):
                    was_tiled = 1.0
        except Exception:
        ...
        # Loop index normalized
        raw_loops = program_annot.get("iterators", [])
        if isinstance(raw_loops, dict):
            all_loops = list(raw_loops.keys())
        else:
            all_loops = list(raw_loops)
        try:
            loop_index = float(all_loops.index(loop_name))
            loop_index_norm = loop_index / max(1.0, float(len(all_loops) - 1))
        except Exception:
            loop_index_norm = 0.0
        
        # Fill feature vector (8-D)
        # 0: depth normalized, 1: is_parallel, 2: has_unroll, 3: was_tiled
        # 4: loop_index_norm, 5: num_children_norm, 6: is_root, 7: reserved
        feat[0] = float(depth_val) / max(1.0, float(max_depth))
        feat[1] = is_parallel
        feat[2] = has_unroll
        feat[3] = was_tiled
        feat[4] = loop_index_norm
        feat[5] = float(num_children) / max(1.0, 8.0)
        feat[6] = 1.0 if depth_val == 0 else 0.0
        # feat[7] left as 0.0 reserved
    ...
    for c_name in sorted(computations_dict.keys(),
        ...
        comp_dict = computations_dict[c_name]
        
        # 0: reduction flag
        if comp_dict.get("comp_is_reduction", False):
            feat[0] = 1.0
        
        # 1: absolute order normalized
        try:
            abs_order = float(comp_dict.get("absolute_order", 0))
            # max order across computations
            max_order = 1.0
            try:
                max_order = float(max(v.get("absolute_order", 0) for v in computations_dict.values()) or 1.0)
            except Exception:
                max_order = 1.0
            feat[1] = abs_order / max_order
        except Exception:
            feat[1] = 0.0
        
        # 2: number of iterators normalized by a small constant (MAX_DEPTH)
        iters = comp_dict.get("iterators", [])
        feat[2] = float(len(iters)) / max(1.0, float(MAX_DEPTH))
        
        # 3-4: proxy fan-in / fan-out if tensors info present
        try:
            inputs = comp_dict.get("tensors_read", [])
            outputs = comp_dict.get("tensors_written", [])
            feat[3] = float(len(inputs)) / 8.0
            feat[4] = float(len(outputs)) / 8.0
        except Exception:
            pass
        
        # 5: part of fusion group
        in_fusion = 0.0
        if isinstance(schedule_json, dict) and schedule_json.get("fusions"):
            try:
                for fus in schedule_json.get("fusions", []):
                    if c_name in fus:
                        in_fusion = 1.0
                        break
            except Exception:
        ...
        # 6-9: schedule flags for this comp
        sched = schedule_json.get(c_name, {}) if isinstance(schedule_json, dict) else {}
        feat[6] = 1.0 if sched.get("vectorize", False) else 0.0
        feat[7] = 1.0 if sched.get("unrolling_factor") else 0.0
        feat[8] = 1.0 if sched.get("shiftings") else 0.0
        feat[9] = 1.0 if sched.get("tiling") else 0.0
        
        # Remaining dims reserved (0.0)
...
# Removed unused stub functions - features are now built inline in build_gnn_data_for_schedule
...
class GNNDatasetParallel:
    def __init__(
        ...
        spawn processes to create GNN data, store them as individual .pt files.
        Else, we load from existing .pt files in pkl_output_folder.
        """
        ...
        if just_load_pickled:
            ...
            # Also load metadata from pkl files if available
            pkl_files = sorted(Path(pkl_output_folder).glob("gnn_representation_part_*.pkl"))
            for pkl_file in pkl_files:
                try:
                    with open(pkl_file, 'rb') as f:
                        local_list = pickle.load(f)
                    # local_list is [(function_name, [(filename,attrs), (filename,attrs), ...])...]
                    for fn, data_attr_pairs in local_list:
                        for (filename, attrs) in data_attr_pairs:
                            self.attr_list.append(attrs)
                except Exception as e:
                    print(f"[WARNING] Failed to load metadata from {pkl_file}: {e}")
            
            # Ensure data_list and attr_list have same length
            if len(self.attr_list) == 0:
                # If no metadata available, create dummy attributes
                self.attr_list = [("unknown", "0000", "", 0.0, 0, "unknown", "<gnn_footprint>", 1.0)] * len(self.data_list)
            elif len(self.data_list) != len(self.attr_list):
                print(f"[WARNING] Mismatch between .pt files ({len(self.data_list)}) and metadata ({len(self.attr_list)})")
            ...
            try:
                ...
                for i in range(nb_processes):
                    ...
                    subset_path = os.path.join(temp_dir, f"subset_{i}.pkl")
                ...
                for i in range(nb_processes):
                    ...
                    for fn, data_attr_pairs in local_list:
                        for (filename, attrs) in data_attr_pairs:
                            full_path = os.path.join(pkl_output_folder, filename)
...
def get_schedule_str(program_json, sched_json):
    ...
    if ("fusions" in sched_json and sched_json["fusions"]):
        for fusion in sched_json["fusions"]:
            ...
            sched_str += ")"
    for name in comp_name:
        raw_iterators = program_json["computations"][name]["iterators"]
        # Handle case where iterators might be a dict instead of a list
        if isinstance(raw_iterators, dict):
            transf_loop_nest = list(raw_iterators.keys()).copy()
        else:
            transf_loop_nest = raw_iterators.copy()
        ...
        if ("fusions" in sched_json and sched_json["fusions"]):
            for fusion in sched_json["fusions"]:
                if name in fusion:
                    ...
                    raw_iterators_fusion = program_json["computations"][iterator_comp_name]["iterators"]
                    # Handle dict case for fusion iterators too
                    if isinstance(raw_iterators_fusion, dict):
                        transf_loop_nest = list(raw_iterators_fusion.keys()).copy()
                    else:
                        transf_loop_nest = raw_iterators_fusion.copy()
